...
var app = angular.module("myFields", []); 
app.controller("myCtrl", function($scope) {
	$scope.fieldsid =[];		// get hakem field  ids
    $scope.fieldsname =[];      // get hakem field  names
    $scope.Allfields = [];
    ...
    $.ajax({
    	...
    	success: function(response){
    		...
    		for(i=0; i<response.data.length; i++){
    			$scope.fieldsid.push(response.data[i].field_id);
        		$scope.fieldsname.push(response.data[i].field_name);
    		}
    	}
    });
